...
`"continue"` is just "first breakpoint greater than `line`" — the [right border](#find-right-border)
search from above, with the `<=` comparison so that a debugger already sitting on a breakpoint moves
to the *next* one instead of staying put. `codeLength` never enters the logic; the guarantees mean
you never run off either end of the list.
